...
onready var colors = [color_night, color_dawn, color_day, color_dusk]
...
#	Global.DayNight = self
	...
	if current_day_hour >= state_night_start_hour: cycle = cycle_state.NIGHT
	elif current_day_hour >= state_dusk_start_hour: cycle = cycle_state.DUSK
	elif current_day_hour >= state_day_start_hour: cycle = cycle_state.DAY
	elif current_day_hour >= state_dawn_start_hour: cycle = cycle_state.DAWN
	else: cycle = cycle_state.NIGHT
...
func day_cycle():
	var last = cycle_state.keys()[cycle]
	current_day_hour = current_time / (day_duration / 24)
	...
	var c
	if current_day_hour >= state_night_start_hour: c = cycle_state.NIGHT
	elif current_day_hour >= state_dusk_start_hour: c = cycle_state.DUSK
	elif current_day_hour >= state_day_start_hour: c = cycle_state.DAY
	elif current_day_hour >= state_dawn_start_hour: c = cycle_state.DAWN
	else: c = cycle_state.NIGHT

	cycle_test(c)



	if debug_mode && last != cycle_state.keys()[cycle]:
